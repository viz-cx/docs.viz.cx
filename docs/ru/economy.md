...
Ликвидные токены можно моментально обменять на долевые, но конвертация обратно займет 28 дней (см. [Понижение доли](#shares-reduction)). При этом коэффициент конвертации в одну и другую сторону равен 1:1 (1 shares за 1 viz и наоборот).
...
Данное распределение эмиссии [установлено делегатами](./witnesses.md##inflation-witness-percent) и может меняться. Консенсусное решение для этого не требуется, проценты определяются как медианное значение параметров, установленных делегатами, которое периодически обновляется.
